...
More formally the model is composed of a population $Pop$ of $m$ agents, each defined by 2 vectors of size $r$. The first corresponds to the quantity of each good that the agent $i$ possesses: $$ Q^i = (q^i_1,\cdots,q^i_r) $$
...
$$ 1- \dfrac{\abs{q^i_j - n_j}}{ \sqrt{\abs{(q^i_j)^2-(n_j)^2}}} $$
...
With such a setup it is expected that the prices will converge to a ratio allowing each agent to obtain a number of resources equivalent to the needs. Knowing that,  the best possible price of a good should follow the equation :

$$\frac{V^j_i}{V^j_g} = N_i   $$
with g the produced good of the agent j and i another good which is not the one which is produced.

If such a price is reached to score of all agents should be equal to : $$ 10\times1\times(ngoods-1) $$
...
0.128&1.98&(0.02)\\
	\end{tabular}
	\caption{Mean \& SD of $\alpha$ parameter }
	\label{tab:Bmualpha}
\end{table}
...
\begin{table}[h]
	\centering
	\begin{tabular}{l|c|c}
	 Imitation  Process :& Rand & Eco \\
	 Mutation Process& & \\\hline
		$\mu$ &A & B \\
		$\mu +\alpha$ & C & D \\
	\end{tabular}
...
\begin{table}[H]
	...
	\begin{tabular}{l c c}
		...
		$\mu$ & \specialcell{\raisebox{-.5\height}{\includegraphics[width=5cm]{img/scoreWrtTimeA.png}}\\Setup A}&\specialcell{\raisebox{-.5\height}{\includegraphics[width=5cm]{img/scoreWrtTimeC.png}} \\ Setup B} \\
		$\mu + \alpha$& \specialcell{\raisebox{-.5\height}{\includegraphics[width=5cm]{img/scoreWrtTimeB.png}} \\ Setup C}&\specialcell{\raisebox{-.5\height}{\includegraphics[width=5cm]{img/scoreWrtTimeD.png}} \\ Setup D}
