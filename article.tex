...
\todo{complete the cite for the method}
In order to verify if the distribution is in fact a power-law, a method is proposed in~\cite{}. Within this method a linear regression is first used, for each run, to fit the curve obtained to a function of the form $P(v)=C/v^\alpha $. A number of fictive data points are then generated based on this slope. The final step assess the validity of the regression by comparing the real distribution and the fictive distribution. The table ~\ref{tab:powerLawFit} summarizes the results obtained by this method on all setups used for this work. The first number of each cell represents the mean value of all $\alpha$ parameters obtained (across the 100 runs of one setup). The second number (in parenthesis) represents the mean value of all statistical tests performed.

\todo{complete table}
\begin{table}[h]
	\centering
	\begin{tabular}{l|lll}
		Innovation rate &\multicolumn{3}{c}{Number of goods}\\
			& 1 & 2 &  5  \\\hline
		0.004 & ??  & ??  & ?? \\
		0.016 & ??  & ??  & ?? \\
		0.064 & ??  & ??  & ?? \\\hline
	\end{tabular}
	\caption{Mean $\alpha$ and p-value for all innovation rate and number of goods studied.}
	\label{tab:powerLawFit}
\end{table}
...
\todo{make the figure and comment the line}
\begin{figure}[h]
	\begin{center}
		\begin{tabular}{cc}
		a) & b)  \\
		\includegraphics[width=6cm]{img/2SetupDistribA.pdf} &
		\includegraphics[width=6cm]{img/2SetupDistribB.pdf} \\
		\includegraphics[width=6cm]{img/2SetupDistribC.pdf} &
		\includegraphics[width=6cm]{img/2SetupDistribD.pdf} \\
		c) & d)  \\
		\end{tabular}

	\end{center}
	\caption{Frequency distribution, where each points represent the mean for 100 runs, for: a) the neutral and the trading models.  b) the neutral model and the trading model without the trading innovation process. c) the trading model and the random model with an innovation process similar than the one used in the trade model. d) the trade model and the trade model without the trading innovation process.}
	\label{fig:2setDi}
\end{figure}
...
In order to investigate which mechanism is responsible for this deviation from the neutral model, additional investigation have been performed. The one presented here consisted in creating a new model composed of the cultural transmission of the ``neutral model'' and the innovation of the ``trade model''. This new model is termed ``composed model''. The figure~\ref{fig:2setDi}.b) presents the results obtained from 100 runs for the ``composed model'' and the ``neutral model''. All runs rely on the same experimental setup as for figure~\ref{fig:2setDi}.a). 

On figure~\ref{fig:2setDi}.b) it appears that the same plateau in the common prices is observed with the ``composed model'' than with the ``trade model''.
We can therefore trace the presence of this plateau to the \emph{innovation} process used in the trading model. This mechanism prevents the creation of totally random new price and therefore leads to the creation of few similar prices obtaining high scores.
