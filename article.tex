...
For comparison purposes the results obtained by~\cite{bentley_random_2004} (which tested the ``neutral hypothesis'' with the same methodology) are added in the last column of table~\ref{tab:mualpha}. It appears that our results are highly similar to those obtained by~\cite{bentley_random_2004}. We note also that these results match the ones shown in~\cite{mesoudi_random_2009} where only one value of $\mu$ was tested ($mu = 0.008$). However, it is difficult to know if the slight differences observed between our work and those previous studies are statistically significant as the two previous studies rely on only five runs for the computation of the mean of $\alpha$ (against 100 in our case).

Nonetheless, for high value of $\mu$, previous works report that the distribution of variant follow a power law~\cite{bentley_random_2004}. This claim is justified based on the fact that the estimated $\alpha$ (estimated using linear regression on the log-log curve) has a high correlation coefficient. More recent works have shown that this method should be avoided~\cite{clauset2009powerlawdistributionsinempiricaldat}. Following these recent findings, our results preclude us to assume that the distribution of variant when $\mu$ is up to $0.064$ follow a power law . 

A series of experiments are performed to see how the system reacts when multiple goods are present. We report in table~\ref{tab:multiGoods} the mean values of $\alpha$ and $p$-value obtained for 3 innovation rates (0.004, 0.016, 0.064) 4 number of goods (1, 3, 6, 9) and 250 agents. 
...
From the results shown in table~\ref{tab:multiGoods} it appears that for each innovation rate the distribution of variants has the same properties regardless of the number of goods. Additional analysis (not shown here) reveals that when the repartition of the price of every goods in a simulation with 9 goods is plotted, the 9 curves are superposed. Moreover these 9 curves are also superposed with the distribution shown in figure~\ref{fig:allMutation} for one good.
