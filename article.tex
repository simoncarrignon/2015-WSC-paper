...
The first set of implementations is designed to reproduce a scenario of unbiased transmission, where each good is a cultural trait without intrinsic positive or negative weight \shortcite{bentley_random_2004,bentley_specialisation_2005,mesoudi_random_2009}. 
...
The \emph{score} $s^i$ of an agent $i$ reflects the ability of this agent to obtain the goods it needs. It is maximum when the quantity vector of an agent is equal to its need vector and lower proportionally to the distance between the need vector and the quantity vector.  It is formally computed as follow for agent $i$:
...
For the cultural transmission model we run experiments with two setups. The first is composed of 1 good and two population sizes: 250 and 500 agents ; the second is composed of 3 goods and 500 agents. For both of these setups 4 different values of $\alpha$ are studied: $\{0.004,0.016,0.064\}$. For each $\alpha$ we present the results obtained with 100 runs of 10000 timestep each. 

For the trading model we run also all experiments with 3 goods and 500 agents. Again 100 runs of 10000 timesteps are performed. As described in section~\ref{sec:trade}, one timestep is corresponds to the call of the \emph{cultural transmission} and \emph{innovation} processes. 
...
We first analyse the result obtained in the ``neutral model" as presented in section~\ref{sec:culturalTrans}. Figure~\ref{fig:allMutation} presents the results obtained with $\mu$ varying $\in {.004,.008,.016,.064}$, 1 good and two population sizes: 250 and 500 agents. The distribution of variants obtained is shown in figure~\ref{fig:allMutation}. In this graph the relative frequencies of the values obtained is analysed. The y-axis of the graph shows the frequencies of the variants of the vector values, the x-axis shows how many variant achieves such frequencies and the line represents the average of the frequencies obtained across all runs. Both axes are in a logarithmic scale.
...
We can see in this table that our results are close to those obtained by \shortcite[3]{bentley_random_2004}. In the same way \shortcite[3]{mesoudi_random_2009} also find similar value of $\alpha$ for similar values of $\mu$ and for the same number of agents. In the other hand it is difficult to know if the slight difference between those study are statistically significant as for the two previous studies the mean of $\alpha$ are computed using only between five runs, against 100 in our case.
...
\begin{figure}[H]
	...
	\end{center}
	\caption{Evolution of the mean of the difference between the estimated value $v_g$ and the optimal value $\tilde{v}_g$ (as calculated with the equations~\ref{eq:optimum}) of the good $g$. This mean is plotted for 100 runs with 3 goods and 500 agents.}
	\label{fig:ratioEvol}
\end{figure}

We observe that prices are indeed converging to the optimal prices which means that our innovation and imitation process are valid trade mechanisms. Notably, they produce results similar to the ones obtained in~\shortcite[3]{gintis_emergence_2006}.
...
The model was created using Pandora \shortcite{rubiocampillo_2014}. R was used for figures and statistical analysis \shortcite{rdev_2012}. The simulations have been done in the supercomputer MareNostrum at Barcelona Supercomputing
Center - Centro Nacional de Supercomputación (The Spanish National Supercomputing Center). The source code of the model is licensed under a GNU General Public License and will be available for download at publication.
