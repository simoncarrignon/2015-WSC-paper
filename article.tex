...
\begin{figure}[htp]
	...
	\label{fig:fit}
\end{figure}
We can see in the figure~\ref{fig:fit} the score with regards to the quantity of one good traded by an agent and that given different value for the need of its good. We can see that even if the need for a good is small (for instance when $need = .01$) one can have a high score as soon as he manage to get a quantity closed to the needed quantity. It insure that one need to have all of each good in the good proportion and that none could reach a better score just managing to get the right amount of a good with a high need.
...
With such a setup and given (\ref{eq:score}) and (\ref{eq:trade}), it is expected that the prices will converge to value allowing each agent to obtain quantities of resources exactly equivalent to the needs. The best possible price of all good satisfies the equations :
\begin{equation}
	\begin{cases}
...
In order to study a trading model two new innovation and imitation mechanisms have been introduced as explained in section~\ref{sec:}. These mechanisms are summarized in the table~\ref{tab:exp}. Each couple of innovation/imitation mechanism form one setup that is labelled with one letter. Note that the ``neutral model" studied in the previous section corresponds to the setup A. All the runs presented in this section are done for 200000 steps with a innovation rate $\mu=.004$ and $\beta=.001$.
...
\begin{table}[H]
	...
	\begin{tabular}{l c c}
		...
		$\mu + \alpha$& \specialcell{\raisebox{-.5\height}{\includegraphics[width=5cm]{img/ScoreEvolutionForC-G2N250.pdf}} \\ Setup C}&\specialcell{\raisebox{-.5\height}{\includegraphics[width=5cm]{img/ScoreEvolutionForD-G3N500.pdf}} \\ Setup D}
...
\begin{figure}[htp]
	...
	\end{center}
	\caption{Evolution of the Score for the four types of agents of a simulation with 4 differents good and 800 agents}
